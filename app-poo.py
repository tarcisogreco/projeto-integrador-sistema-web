...
class BalancoEnergiaApp:
    def __init__(self):
        ...
        self.graficos = GraficosEnergia(self.siglas_subsistema)
        self.df = self.carregador.carregar()
    ...
    def run(self):
        if self.df is not None:
            ...
            self.graficos.grafico_pizza(sizes, sigla_desejada)
            self.graficos.grafico_barras_tipo(sizes, soma_geracao, sigla_desejada)
            values = [soma_geracao, soma_carga, intercambio]
            self.graficos.grafico_barras_balanco(values, sigla_desejada)
